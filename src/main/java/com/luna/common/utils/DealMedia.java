...
public class DealMedia {
     ...
     * @param prefix 图片路径前缀
     * @param outputPath 视频文件路径存储路径 包括文件名和类型
     * @param ffmpegPath ffmpeg所在文件夹
     * @param inputType 输入图片名称
     * @param totalTime 合成时长
     * @param frame 每秒帧数
     */
    public static void coding(String inputPath, String prefix, String outputPath, String ffmpegPath, String inputType,
        String totalTime, String frame) {
        DealMedia.totalTime = totalTime;
        List<String> command = getCodingCommand(inputPath, prefix, outputPath, ffmpegPath, inputType, frame);
     ...
     * @throws IOException
     */
    public static void decode(String inputPath, String outputPath, String prefix, String ffmpegPath, String outputType)
        throws IOException {
        List<String> command = getDecodeCommand(inputPath, outputPath, ffmpegPath, outputType, prefix);
        if (null != command && command.size() > 0) {
            if (!process(command)) {
    ...
    private static List<String> getDecodeCommand(String inputPath, String outputPath, String ffmpegPath,
        String outputType, String prefix) {
        List<String> command = new ArrayList<String>();
        command.add(ffmpegPath);
        command.add("-ss");
        /** 起始时间 */
